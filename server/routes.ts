...
【重要な処理手順】
1. まず入力されたテキストの言語を自動検出してください。
2. 日本語以外の場合（英語、ベトナム語、中国語、韓国語など）は、正確に日本語に翻訳してください。
3. 翻訳後または日本語の場合、以下のルールで文章を修正してください：
   - 記録に存在しない情報は一切追加しない
   - 人名が入っている場合は人名をひらがなにして、敬称は「さん」にする
   - 彼や彼女など記録の対象者を指す言葉は取り除く
   - 看護用語、介護用語の誤変換と想定される単語は正しい看護用語、介護用語に直す
   - （例）正式畳→清拭畳
4. 一度修正した文章を見直し、必要な場合は再度修正を行ってから返してください。

最終的に日本語の看護・介護記録として適切な文章を返してください。`
          },
          {
            role: "user",
            content: inputText
